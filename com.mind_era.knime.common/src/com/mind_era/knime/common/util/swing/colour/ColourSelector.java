...
import com.mind_era.knime.common.util.JEPHelper;
...
public class ColourSelector extends JPanel implements HasMetaModel {
	...
	public static enum RangeType implements Displayable {
		 ...
		 * (non-Javadoc)
		 * 
		 * @see com.mind_era.knime.common.util.Displayable#getDisplayText()
		 */
		@Override
		...
		private static final class ComplexColourPanel extends JPanel implements
			...
			private static abstract class SpinnersCommon extends
				...
				protected SpinnersCommon(final Pair<String, String> key) {
					super();
					lowTextField = new JTextField(key.getFirst(),
					...
					highTextField = new JTextField(key.getSecond(),
							ComplexColourPanel.SPINNER_COLUMNS);
			...
			private void setModel(final ComplexMetaModel model) {
				...
				if (bigChange) {
					...
					for (final Pair<Pair<String, String>, ?> entry : entries) {
						...
						if (entry.getSecond() instanceof Pair<?, ?>) {
							...
							if (pair.getFirst() instanceof Color
								...
								spinner = new ContinuousControl(
										entry.getFirst(), colours.getFirst(),
										colours.getSecond());
							} else {
						...
						} else if (entry.getSecond() instanceof Color) {
							final Color colour = (Color) entry.getSecond();
							spinner = new DiscreteControl(entry.getFirst(),
									colour);
						} else {
							throw new IllegalStateException(
									"Only gradient and single colours are supported."
											+ entry.getSecond().getClass());
						}
			...
			for (final Component comp : getComponents()) {
				if (comp instanceof ColourControl<?>) {
					...
					for (final Pair<Pair<String, String>, Object> entry : metamodel.entries) {
						final JEP jep = new JEP();
						jep.setImplicitMul(true);
						final String lowStr = entry.getFirst().getFirst();
						final String highStr = entry.getFirst().getSecond();
						...
						try {
							...
							final Double highVal = (Double) jep
									.evaluate(highNode);
							if (entry.getSecond() instanceof Color) {
								final Color color = (Color) entry.getSecond();
							...
							if (entry.getSecond() instanceof Pair<?, ?>) {
								...
								final Pair<Color, Color> pair = (Pair<Color, Color>) entry
										.getSecond();
								assert pair.getFirst() instanceof Color : pair;
								assert pair.getSecond() instanceof Color : pair;
